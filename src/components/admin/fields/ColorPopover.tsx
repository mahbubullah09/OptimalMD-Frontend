...
export const BRAND_SOLIDS: {
  ...
  {
    label: "Brand blue",
    spec: { kind: "tone", tone: "blue" },
    swatch: "#1FA9E8",
  },
  {
    label: "Brand green",
    spec: { kind: "tone", tone: "green" },
    swatch: "#5BA84A",
  },
  {
    label: "Navy",
    spec: { kind: "solid", color: "#0B2545" },
    swatch: "#0B2545",
  },
  {
    label: "White",
    spec: { kind: "solid", color: "#FFFFFF" },
    swatch: "#FFFFFF",
  },
];
...
const inherited = (
  ...
  if (device === "mobile")
    return size.mobile ?? size.tablet ?? size.desktop ?? null;
...
export default function ColorPopover({
  ...
  const setSize = (value: number, nextUnit: SizeUnit) =>
    onSize({
      ...activeSize,
      [device]: { value: clampSize(value, nextUnit), unit: nextUnit },
    });
  ...
  const presetKeys = new Set(
    [...BRAND_SOLIDS, ...BRAND_GRADIENTS].map((p) => specKey(p.spec)),
  );
    ...
    <div className="fmtPop" ref={wrapRef}>
      ...
      >
        ...
        <span
          ...
          style={{
            background: activeSpec ? swatchFor(activeSpec) : "transparent",
          }}
        />
      </button>
      ...
      <Popover open={open} anchorRef={wrapRef} onClose={() => setOpen(false)}>
        ...
        <p className={`fmtScope${scope === "field" ? " isField" : ""}`}>
          {SCOPE_NOTE[scope]}
        </p>
        ...
        <p className="fmtSizeNote">
          {sizeMixed ? (
            ...
            <>
              ...
              >
                Match{" "}
                {device === "desktop" ? "the normal size" : "wider screens"}
              </button>
            </>
          ) : device === "desktop" ? (
